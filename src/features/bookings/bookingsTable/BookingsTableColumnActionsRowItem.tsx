...
export default function BookingsTableColumnActionsRowItem({
	...
	const { data: bookingStatus, error } = BookingsStatusSchema.safeParse(booking.status);
	...
	return (
		<DropdownMenu>
			...
			<DropdownMenuContent className="w-56">
				...
				<DropdownMenuGroup>
					<DropdownMenuItem
						onMouseEnter={prefetchBookingQuery}
						render={
							<Link
								href={$path({
					...
					{bookingStatus && bookingStatus !== 'checked-out' ? (
						<DropdownMenuItem
							...
							render={
								<button
									className="w-full"
									onClick={bookingStatusHandler[bookingStatus].onClickHandler}
								>
