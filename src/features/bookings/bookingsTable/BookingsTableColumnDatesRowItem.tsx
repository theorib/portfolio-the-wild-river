...
import useBookingDates, { type BookingDatesInput } from '@/features/bookings/hooks/useBookingDates';

type BookingsTableColumnDatesProps = {
	bookingDates: BookingDatesInput;
};

export default function BookingsTableColumnDates({ bookingDates }: BookingsTableColumnDatesProps) {
	const { data, success, error } = useBookingDates({ bookingDates });

	if (error) {
		return <BookingTableItemCellError error={error}>Invalid Dates</BookingTableItemCellError>;
	}
		...
		return (
			<BookingsTableItemCellContainer className="grid grid-cols-[max-content_max-content_max-content]">
				<div className="contents">
					...
					<BookingTableItemCellBold>{stayLength} night stay</BookingTableItemCellBold>
				</div>
				<div className="contents">
