...
function TodaySummaryItem({ className, ...props }: React.ComponentProps<'li'>) {
	return <li data-slot="today-summary-item" className={cn('contents', className)} {...props} />;
}
...
function TodaySummaryItemBadge({ bookingStatus, className, ...props }: TodaySummaryItemBadgeProps) {
	const color = {
	...
	return (
		<Badge
			...
			className={cn('inline-flex w-full uppercase', color[bookingStatus], className)}
			{...props}
		/>
	);
}
function TodaySummaryItemFlag({ className, ...props }: React.ComponentProps<'span'>) {
	return (
		<span data-slot="today-summary-item-flag" className={cn('text-3xl', className)} {...props} />
	);
}
function TodaySummaryItemName({ className, ...props }: React.ComponentProps<'span'>) {
	return <span className={cn('text-base', className)} {...props} />;
}

function TodaySummaryItemNumber({ className, ...props }: React.ComponentProps<'span'>) {
	return <span className={cn('justify-self-center text-base', className)} {...props} />;
}
function TodaySummaryItemButton({ className, ...props }: React.ComponentProps<'button'>) {
...
function TodaySummaryTitle({ className, ...props }: React.ComponentProps<'p'>) {
	return (
		<p
			...
			className={cn('tracking-tights text-xl leading-none font-semibold', className)}
			{...props}
		/>
	);
}

function TodaySummaryContent({ className, ...props }: React.ComponentProps<'div'>) {
	return (
		<div
...
function TodaySummary({ className, ...props }: React.ComponentProps<'div'>) {
	return (
		<div
			...
			className={cn('bg-card text-stats-foreground rounded-xl border shadow-sm', className)}
			{...props}
		/>
	);
}
