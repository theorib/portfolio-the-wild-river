...
export class NextRateLimiter {
	...
	static getInstance(options?: Partial<NextRateLimiterOptions>): NextRateLimiter {
	...
	async consume(ip: string): Promise<NextRateLimiterResponse> {
		try {
			...
			return this.createResponse({
				...
				duration: this.options.duration ?? NextRateLimiter.defaultOptions.duration,
	...
	private createResponse(options: NextRateLimiterResponsePayload): NextRateLimiterResponse {
		const { points, duration, success, status, message, rateLimiterRes } = options;
	...
	private isNextRateLimiterResponse(rateLimiterRes: unknown): rateLimiterRes is RateLimiterRes {
		return (
	...
	static validateOptions(options?: Partial<NextRateLimiterOptions>): NextRateLimiterOptions {
