...
async function deleteGuests() {
	...
	const { error, count, statusText, status } = await supabase.from('guests').delete().gt('id', 0);
	if (error) {
...
async function deleteCabins() {
	...
	const { error, count, statusText, status } = await supabase.from('cabins').delete().gt('id', 0);
	if (error) {
		logger
			.withMetadata({
		...
		logger.withContext({ statusText, status, count }).info(`Deleted ${count} cabins`);
	}
}

async function deleteBookings() {
	logger.info('Deleting bookings...');
	const { error, count, statusText, status } = await supabase.from('bookings').delete().gt('id', 0);
	if (error) {
		logger
			.withMetadata({
...
async function createGuests() {
	const { error, count, statusText, status } = await supabase.from('guests').insert(guests);
	if (error) {
		logger
			.withMetadata({
		...
		logger.withContext({ statusText, status, count }).info(`Created ${count} guests`);
	}
}

async function createCabins() {
	const { error, status, count, statusText } = await supabase.from('cabins').insert(cabins);
	if (error) {
		logger
			.withMetadata({
		...
		logger.withContext({ statusText, status, count }).info(`Created ${count} cabins`);
	}
}
...
async function createBookings() {
	...
	const finalBookings = bookings.map((booking) => {
		...
		if (!cabin) {
			const error = new Error(`Cabin not found for booking with cabinId: ${booking.cabinId}`);
			logger
				.withMetadata({
